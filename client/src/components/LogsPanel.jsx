...
const ICONS = { system: '🖥', auth: '🔐', control: '🎛', pms: '📋', guest: '👤', telemetry: '📡', sensor: '📟', service: '🛎' };
...
export default function LogsPanel() {
  ...
  const exportCSV = () => {
    ...
    // Trigger as a direct download via a temporary anchor
        ...
        {filtered.map((e, i) => {
          ...
          return (
            <div key={`${e.ts}-${i}`} className="flex items-start gap-2 px-3 py-2 border-b border-gray-50">
              ...
              <span className="text-xs">{ICONS[e.cat] || '📌'}</span>
              <div className="flex-1">
                ...
                {e.room && <span className="ml-1.5 text-[8px] font-bold bg-gray-100 px-1.5 py-0.5 rounded font-mono">Rm{e.room}</span>}
              </div>
            </div>
          );
        })}
