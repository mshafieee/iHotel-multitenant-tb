...
const STATUS_LABELS = ['Vacant', 'Occupied', 'MUR', 'Maintenance'];
const STATUS_COLORS = ['#16A34A', '#2563EB', '#D97706', '#DC2626'];
const STATUS_BG     = ['#DCFCE7', '#DBEAFE', '#FEF3C7', '#FEE2E2'];
...
export default function KPIRow({ role }) {
  ...
  const stats = useMemo(() => {
    ...
    const alerts = rooms.filter(r => r.sosService || !r.online).length;
    const avgTemp = +(rooms.reduce((s, r) => s + (r.temperature || 22), 0) / n).toFixed(1);
    const rev = rooms.filter(r => r.roomStatus === 1).reduce((s, r) => s + (RATES[r.type] || 0), 0);
    ...
    return { n, occ, or, alerts, avgTemp, rev, mur, dnd, dist };
  }, [rooms]);
  ...
  const kpis = [
    ...
    { icon: '🚨', label: 'Alerts', value: stats.alerts, color: stats.alerts > 0 ? 'text-red-500' : 'text-emerald-500' },
          ...
          <div className="flex-1 space-y-1.5">
            {stats.dist.map(d => (
              <div key={d.label} className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-gray-600 w-24 shrink-0">{d.label}</span>
